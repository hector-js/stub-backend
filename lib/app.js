...
try {
  const argsFile = fs.readFileSync('./.hjs.config.json').toString();
  try {
    argsFile = JSON.parse(argsFile);
  } catch (err) {
    throw new Error('.hjs.config.json is not well-formed');
  }
  port = checkArg(argsFile.port, port);
  logs = checkArg(argsFile.logs, logs);
  corsArgs = checkArg(argsFile.corsArgs, corsArgs);
} catch (err) {
