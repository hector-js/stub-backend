...
module.exports = resultHandler = (scenario, req) => {
  ...
  if (scenario && !scenario.message) {
    const headersInValid = ReqUtils.hasInvalidElements(scenario._headers, req.headers);
    const cookiesInValid = ReqUtils.hasInvalidElements(scenario._cookies, req.cookies);
