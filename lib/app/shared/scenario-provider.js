...
module.exports = class ScenarioProvider {
  ...
  filterByRequest(req, scenarios) {
    ...
    const scenario = scenarios.find((sce) =>
      isJson(body) ?
        DBUtils.compareJSON(sce._req['_body'], body, sce._req['_excludeBodyFields']) :
        DBUtils.compareXML(sce._req['_body'], body.toString()));

    if (!scenario) {
      const noReqBodyScenarios = scenarios.filter((s) => !s._req['_body']);
